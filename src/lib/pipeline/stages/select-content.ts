...
export function createSelectContentStage(
  options: SelectContentStageOptions = {},
): PipelineStage {
  return {
    name: "select_content",
    async execute(
      ...
      try {
        ...
        if (options.mediaId) {
          ...
          if (!mediaItem) {
            mediaItem =
              library.tvShows.find((item: { path: string }) => item.path === options.mediaId) ?? null;
          }
        } else if (options.mediaPath) {
          const mediaType = await libraryScanner.detectMediaType(options.mediaPath);
          if (mediaType === "movie") {
            const parentDir = options.mediaPath.substring(
              0,
              options.mediaPath.lastIndexOf("/"),
            );
            const movies = await libraryScanner.scan(parentDir);
            mediaItem = movies.movies.find((m: { path: string }) => m.path === options.mediaPath) ?? null;
          }
        }

...
describe('LlmAnalyzer', () => {
  it('returns structured results for the movie', async () => {
    aiMocks.generateObject.mockResolvedValue({
      object: {
        movie_title: 'Test Movie',
        video_title: 'Five Secrets Hidden in This Classic Thriller',
  ...
  it('returns null when movie_title mismatches', async () => {
    aiMocks.generateObject.mockResolvedValue({
      object: {
        movie_title: 'Wrong Movie',
  ...
  it('includes full subtitle entries in the prompt', async () => {
    ...
    await analyzer.analyze({ title: 'Test Movie', year: 2023 }, subtitles);

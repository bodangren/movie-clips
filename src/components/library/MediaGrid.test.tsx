...
describe("MediaGrid", () => {
  ...
  it("renders correct grid columns for different sizes", () => {
    const { rerender } = render(
      <MediaGrid items={mockMovies} columns="sm" />
    );
    expect(screen.getByTestId("media-grid")).toHaveClass("grid-cols-2", "sm:grid-cols-3", "lg:grid-cols-4", "xl:grid-cols-5");

    rerender(<MediaGrid items={mockMovies} columns="lg" />);
    expect(screen.getByTestId("media-grid")).toHaveClass("grid-cols-1", "sm:grid-cols-2", "lg:grid-cols-3");
  });
